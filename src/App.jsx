...
const Column = ({ cards, canPlace, onPlace, showCards, isRevealed, flippedBack }) => (
  <div className={`column-cards ${canPlace ? 'can-place' : ''}`} onClick={canPlace ? onPlace : undefined}>
    {cards.map((card, i) => <Card key={i} card={card} faceDown={!(i === 0 || showCards || isRevealed) || flippedBack} />)}
    {canPlace && cards.length < 5 && <div className="place-indicator">+</div>}
  </div>
);
...
export default function App() {
  ...
  const placeCardLocal = (columnIndex) => {
    ...
    const isP1 = currentPlayer === 1, cols = isP1 ? player1Columns : player2Columns, setCols = isP1 ? setPlayer1Columns : setPlayer2Columns;
    if (cols[columnIndex].length >= 5) return;
  ...
  const createRoom = async () => {
    ...
    try {
      ...
      const success = await firebaseDB.set(`rooms/${code}`, { 
        seed, 
    ...
    <div className="game-container">
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .column-cards{display:flex;gap:1px;padding:2px;border-radius:4px;transition:all .2s ease;position:relative}
              ...
              return (
                <div key={colIndex} className={`duel-row ${isRevealing ? 'revealing' : ''} ${flippedLosers[colIndex] ? (p1Winner ? 'winner-p1' : 'winner-p2') : ''}`}>
                  <div className="player-side">
                    <Column cards={player1Columns[colIndex]} canPlace={gameState === 'placing' && currentPlayer === 1 && player1Columns[colIndex].length < 5} onPlace={() => placeCardLocal(colIndex)} showCards={currentPlayer === 1} isRevealed={isRevealed} flippedBack={flippedLosers[colIndex] && p2Winner} />
                  ...
                  <div className="player-side">
                    <Column cards={player2Columns[colIndex]} canPlace={gameState === 'placing' && currentPlayer === 2 && player2Columns[colIndex].length < 5} onPlace={() => placeCardLocal(colIndex)} showCards={currentPlayer === 2} isRevealed={isRevealed} flippedBack={flippedLosers[colIndex] && p1Winner} />
            ...
            {[0, 1, 2, 3, 4].map(colIndex => {
              ...
              return (
                <div key={colIndex} className={`duel-row ${isRevealing ? 'revealing' : ''} ${flippedLosers[colIndex] ? (myWinner ? 'winner-p1' : 'winner-p2') : ''}`}>
                  <div className="player-side">
                    <Column cards={myColumns[colIndex]} canPlace={gameState === 'placing' && currentPlayer === playerNumber && myColumns[colIndex].length < 5} onPlace={() => placeCardOnline(colIndex)} showCards={true} isRevealed={isRevealed} flippedBack={flippedLosers[colIndex] && oppWinner} />
                  ...
                  <div className="player-side">
                    <Column cards={oppColumns[colIndex]} canPlace={false} showCards={false} isRevealed={isRevealed} flippedBack={flippedLosers[colIndex] && myWinner} />
